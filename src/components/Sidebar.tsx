...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex flex-col bg-sidebar border-r border-sidebar-border transition-[width,transform] duration-300 ${
      ...
      >
        <div className="h-16 flex items-center px-4 border-b border-sidebar-border shrink-0">
          <Link to="/" onClick={handleNav} className="flex items-center gap-3 overflow-hidden">
          ...
          <button
            ...
            className="ml-auto w-8 h-8 hidden lg:grid place-items-center rounded-md hover:bg-sidebar-accent text-muted-foreground hover:text-sidebar-foreground transition"
          ...
          <button
            onClick={closeMobileNav}
            className="ml-auto lg:hidden w-11 h-11 grid place-items-center rounded-md hover:bg-sidebar-accent text-muted-foreground hover:text-sidebar-foreground transition"
          ...
          {(app.isAuthenticated ? authedNavItems : guestNavItems).map((item: any) => {
            ...
            return (
              <Link
                ...
                to={item.to}
                onClick={handleNav}
                className={`w-full flex items-center gap-3 px-3 min-h-[44px] rounded-lg text-sm font-medium transition relative ${
                  isActive
              ...
              {pinnedItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    to={item.to}
                    onClick={handleNav}
                    className={`w-full flex items-center gap-3 px-3 min-h-[44px] rounded-lg text-xs font-medium transition ${
                      isActive
                        ? "bg-sidebar-accent text-sidebar-foreground"
                        : "text-muted-foreground hover:bg-sidebar-accent/60 hover:text-sidebar-foreground"
                    }`}
          ...
          {app.isAuthenticated && !collapsed && (
            <>
              ...
              <button
                ...
                className="w-full flex items-center gap-2 px-3 min-h-[44px] text-[10px] uppercase tracking-wider text-muted-foreground font-medium hover:text-foreground transition"
              >
                <ChevronRight
        ...
        <div className="border-t border-sidebar-border p-3 space-y-2 shrink-0">
          <div className="relative">
            <button
              ...
              className="w-full flex items-center gap-3 px-3 min-h-[44px] rounded-lg text-sm text-muted-foreground hover:bg-sidebar-accent hover:text-sidebar-foreground transition disabled:opacity-50"
              ...
              <div className="absolute bottom-full left-0 right-0 mb-2 glass-strong rounded-lg p-1 shadow-elegant z-50">
                {languages.map((l) => (
                  <button
                    ...
                    className={`w-full text-left px-3 min-h-[44px] rounded-md text-sm transition ${
                      lang === l.code
          ...
          {app.isAuthenticated ? (
            <button
              ...
              className={`w-full flex items-center justify-center gap-2 rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 font-medium text-sm transition min-h-[44px] ${
                collapsed ? "px-0" : "px-3"
              }`}
            ...
            <div className="space-y-2">
              <Link to="/login" onClick={handleNav} className={`w-full flex items-center justify-center gap-2 rounded-lg glass hover:border-white/25 text-foreground font-medium text-sm transition min-h-[44px] ${collapsed ? "px-0" : "px-3"}`}>
                <LogIn className="w-[18px] h-[18px]" />
                {!collapsed && <span>Sign In</span>}
              </Link>
              <Link to="/signup" onClick={handleNav} className={`w-full flex items-center justify-center gap-2 rounded-lg bg-gradient-brand text-primary-foreground font-medium text-sm transition hover:shadow-glow min-h-[44px] ${collapsed ? "px-0" : "px-4"}`}>
