...
import { StatTile, Badge, FakeQR } from "@/components/ui-bits";
      ...
      actions={
        <>
          <span className="inline-flex items-center gap-2 px-3 h-9 rounded-lg glass text-xs">
            <span className="w-2 h-2 rounded-full bg-emerald shadow-glow" /> Stellar Mainnet
          </span>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass rounded-2xl p-6">
          <div className="flex items-start justify-between gap-4 mb-6">
            ...
            {!wallet.connected ? (
              ...
              <Badge tone="success">Connected · {wallet.provider}</Badge>
            )}
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 mt-6">
            ...
            <Link to="/create-campaign" className="h-12 rounded-xl glass hover:border-white/25 font-medium inline-flex items-center justify-center gap-2 transition">
              <PlusCircle className="w-4 h-4" /> Campaign
            </Link>
          </div>
        </div>
        ...
        <div className="glass rounded-2xl p-6 flex flex-col">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <div className="font-medium">Stellar Mainnet</div>
              <div className="text-xs text-muted-foreground">Ledger 52,148,920</div>
            </div>
          </div>
          <div className="space-y-2 text-sm">
            <Row label="Avg confirmation" value="4.2s" />
            <Row label="Avg fee" value="0.00001 XLM" />
            <Row label="Validators" value="178 online" />
          </div>
          <div className="mt-auto pt-6 flex justify-center">
            <FakeQR value={address} size={160} />
          </div>
          <div className="text-center text-[11px] text-muted-foreground mt-2">
            Scan to send XLM to this address
          </div>
        </div>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
        <StatTile label="Total Raised" value="$48,290" delta="+12.4%" icon={Heart} />
        <StatTile label="Active Campaigns" value="7" delta="+2" icon={Megaphone} />
        <StatTile label="Beneficiaries" value="1,284" delta="+38" icon={Users} />
        <StatTile label="Transactions" value="9,420" delta="+5.1%" icon={ArrowLeftRight} />
      </div>
      ...
      <div className="glass rounded-2xl mt-6 overflow-hidden">
        <div className="flex items-center justify-between p-5 border-b border-white/5">
          <div>
            ...
            <div className="text-xs text-muted-foreground">Latest on-chain transactions</div>
          </div>
          <Link to="/transactions" className="text-xs text-primary hover:underline">View all →</Link>
        </div>
        <div className="divide-y divide-white/5">
          {txs.map((t) => (
            <div key={t.id} className="flex items-center gap-4 p-4 hover:bg-white/[0.02] transition">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-muted-foreground truncate">{t.campaign}</div>
              </div>
              <div className="text-right shrink-0">
                <div className={`text-sm font-mono ${t.amount.startsWith("+") ? "text-emerald" : "text-foreground"}`}>
                  {t.amount}
                </div>
                <div className="text-[11px] text-muted-foreground">{t.time}</div>
              </div>
              <Badge
                tone={t.status === "confirmed" ? "success" : t.status === "pending" ? "warning" : "danger"}
              >
                {t.status}
              </Badge>
            </div>
