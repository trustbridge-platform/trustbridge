...
function SendPage() {
  ...
  return (
    <AppShell title="Send" subtitle="Transfer assets to any Stellar address.">
      <form onSubmit={send} className="glass rounded-2xl p-6 max-w-2xl">
        ...
        <div className="mt-6 p-4 rounded-xl bg-white/[0.03] border border-white/5 space-y-2 text-sm">
          <Row label="Amount" value={`${amount} ${form.asset}`} />
          <Row label="Network fee" value={`${fee} XLM`} />
          <Row label="Total" value={`${amount} ${form.asset} + ${fee} XLM`} bold />
        </div>
        ...
        >
          {sent ? "Broadcasting…" : wallet.connected ? (<>Send transaction <ArrowRight className="w-4 h-4" /></>) : (<><Wallet className="w-4 h-4" /> Connect wallet to send</>)}
